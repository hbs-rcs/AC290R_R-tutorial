...
Use the `str()` function to inspect the **str**ucture of an object:
...
the number of columns as the second element (the **dim**ensions of the object)
...
It is easy to read Stata .dta files using the `read_stata()` function from the haven package:
...
Writing to a .dta file can be done using the `write_stata()` function from the haven package:
